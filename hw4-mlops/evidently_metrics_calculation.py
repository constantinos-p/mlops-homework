...
create table dummy_metrics(
	...
	fair_amount_50_percentile float,
	rmse float
)
"""
...
def calculate_metrics_postgresql(curr, i):
	...
	fair_amount_50_percentile = float(result['metrics'][3]['result']['current']['value'])
	rmse = result['metrics'][4]['result']['current']['rmse']
	curr.execute(
		"insert into dummy_metrics(timestamp, prediction_drift, num_drifted_columns, share_missing_values,fair_amount_50_percentile,rmse) values (%s, %s, %s, %s,%s,%s)",
		(begin + datetime.timedelta(i), prediction_drift, num_drifted_columns, share_missing_values,fair_amount_50_percentile,rmse)
	)
